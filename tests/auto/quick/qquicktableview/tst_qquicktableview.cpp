...
void tst_QQuickTableView::checkColumnWidthWithoutProvider()
{
    ...
    for (const int column : qAsConst(tableViewPrivate->loadedColumns).keys()) {
        const qreal expectedColumnWidth = tableViewPrivate->sizeHintForColumn(column);
        for (const int row : qAsConst(tableViewPrivate->loadedRows).keys()) {
...
void tst_QQuickTableView::checkContentWidthAndHeight()
{
    ...
    QCOMPARE(tableViewPrivate->loadedColumns.firstKey(), smallCellCount);
    QCOMPARE(tableViewPrivate->loadedColumns.lastKey(), columnCount - 1);
...
void tst_QQuickTableView::checkPageFlicking()
{
    ...
    QCOMPARE(loadedRows.firstKey(), 0);
    QCOMPARE(loadedColumns.firstKey(), 0);
    ...
    QCOMPARE(loadedRows.firstKey(), 0);
    QCOMPARE(loadedColumns.firstKey(), flickToColumn);
    QCOMPARE(loadedColumns.count(), tableView->width() / cellWidth);
    QCOMPARE(loadedRows.count(), tableView->height() / cellHeight);
    ...
    QCOMPARE(loadedRows.firstKey(), flickToColumn);
    QCOMPARE(loadedColumns.firstKey(), flickToRow);
    QCOMPARE(loadedRows.count(), tableView->height() / cellHeight);
    QCOMPARE(loadedColumns.count(), tableView->width() / cellWidth);
}
...
void tst_QQuickTableView::checkContentXY()
{
    ...
    QCOMPARE(tableViewPrivate->loadedColumns.firstKey(), expectedCellXY);
    QCOMPARE(tableViewPrivate->loadedRows.firstKey(), expectedCellXY);
}
...
void tst_QQuickTableView::fillTableViewButNothingMore()
{
    ...
    auto const bottomRightLoadedCell = QPoint(tableViewPrivate->loadedColumns.lastKey(), tableViewPrivate->loadedRows.lastKey());
...
void tst_QQuickTableView::flick()
{
    ...
    for (qreal cellsToFlick : flickValues) {
        ...
        QCOMPARE(loadedColumns.firstKey(), expectedTableLeft);
        QCOMPARE(loadedColumns.lastKey(), expectedTableLeft + visualColumnCount);
        QCOMPARE(loadedRows.firstKey(), expectedTableTop);
        QCOMPARE(loadedRows.lastKey(), expectedTableTop + visualRowCount);
    }
}
...
void tst_QQuickTableView::flickOvershoot()
{
    ...
    QCOMPARE(loadedColumns.firstKey(), 0);
    QCOMPARE(loadedColumns.lastKey(), 0);
    QCOMPARE(loadedRows.firstKey(), 0);
    QCOMPARE(loadedRows.lastKey(), rowCount - 1);
    ...
    QCOMPARE(loadedColumns.firstKey(), columnCount - 1);
    QCOMPARE(loadedColumns.lastKey(), columnCount - 1);
    QCOMPARE(loadedRows.firstKey(), 0);
    QCOMPARE(loadedRows.lastKey(), rowCount - 1);
    ...
    QCOMPARE(loadedColumns.firstKey(), 0);
    QCOMPARE(loadedColumns.lastKey(), columnCount - 1);
    QCOMPARE(loadedRows.firstKey(), rowCount - 1);
    QCOMPARE(loadedRows.lastKey(), rowCount - 1);
