...
protected:
    ...
    void calculateInitialColumnRowSizes();
    void calculateInitialRowHeights();
...
void QQuickTablePrivate::calculateInitialColumnRowSizes()
{
...
void QQuickTablePrivate::calculateInitialRowHeights()
{
...
void QQuickTablePrivate::calculateColumnWidth(int column)
{
    if (column < columnWidths.last().columnOrRow) {
        ...
        // recalculating the width of a column if flicking it in and out of view.
        return;
    }
...
void QQuickTablePrivate::calculateRowHeight(int row)
{
    if (row < rowHeights.last().columnOrRow) {
        // We only do the calculation once, and then stick with the size. This to avoid
        // recalculating the height of a row if flicked it in and out of view.
        return;
    }
...
qreal QQuickTablePrivate::columnWidth(int column)
{
    if (!columnWidths.isEmpty()) {
        // Find the ColumnOrRowSize assignment before, or at, column
        auto iter = std::lower_bound(columnWidths.constBegin(), columnWidths.constEnd(),
                                     ColumnOrRowSize{column, -1}, ColumnOrRowSize::lessThan);

        // First check if we got an explicit assignment
        if (iter->columnOrRow == column)
            return iter->size;

        // If the table is not a list, return the size of
        // ColumnOrRowSize element found before column.
        if (q_func()->rows() > 1)
            return (iter - 1)->size;
    }
...
qreal QQuickTablePrivate::rowHeight(int row)
{
    if (!rowHeights.isEmpty()) {
        // Find the ColumnOrRowSize assignment before, or at, row
        auto iter = std::lower_bound(rowHeights.constBegin(), rowHeights.constEnd(),
                                     ColumnOrRowSize{row, -1}, ColumnOrRowSize::lessThan);

        // First check if we got an explicit assignment
        if (iter->columnOrRow == row)
            return iter->size;
...
void QQuickTablePrivate::polishTableAfterRebuilding()
{
    ...
    rowHeights.clear();
    if (q->rows() > 1)
        calculateInitialColumnRowSizes();
    if (q->columns() > 1)
        calculateInitialRowHeights();
