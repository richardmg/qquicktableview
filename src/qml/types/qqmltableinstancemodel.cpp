...
{
    deleteAllPostedIncubationTasks();
...
QObject *QQmlTableInstanceModel::object(int index, QQmlIncubator::IncubationMode incubationMode)
{
    ...
    if (modelItem->incubationTask) {
        ...
        // the item async, but the current request needs it sync, we need for force-complete the incubation.
...
void QQmlTableInstanceModel::deleteAllPostedIncubationTasks()
{
    qDeleteAll(m_finishedIncubationTasks);
    m_finishedIncubationTasks.clear();
}

bool QQmlTableInstanceModel::event(QEvent *e)
{
...
void QQmlTableInstanceModel::connectToAbstractItemModel()
{
...
void QQmlTableInstanceModel::disconnectToAbstractItemModel()
{
    auto aim = m_adaptorModel.aim();
    if (!aim)
        return;
...
void QQmlTableInstanceModel::dataChangedCallback(const QModelIndex &begin, const QModelIndex &end, const QVector<int> &roles)
{
    Q_UNUSED(end);
    Q_UNUSED(roles);
    if (begin.parent() != m_adaptorModel.rootIndex)
        return;
...
void QQmlTableInstanceModelIncubationTask::statusChanged(QQmlIncubator::Status status)
{
    ...
    // The model was deleted after we started
    // incubating, so just delete the incubated model item.
    QQmlTableInstanceModel::deleteModelItemLater(modelItemToIncubate);
}
