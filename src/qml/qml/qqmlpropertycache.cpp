...
*/
QQmlPropertyCache::QQmlPropertyCache(const QMetaObject *metaObject, int minorVersion)
    : QQmlPropertyCache()
{
    Q_ASSERT(metaObject);
    ...
    if (minorVersion > 0) {
        // Explicitly set the version of the meta object that this cache describes to be
        // 'minorVersion'. This is mostly useful for constructing a property cache from a
        // type that was created directly in C++ (and not QML) and therefore has no
        // associated import version. For such types, the allowed version for each recorded
        // QMetaObject would normally be zero, which would exclude any revisioned properties.
        for (int metaObjectOffset = 0; metaObjectOffset < allowedRevisionCache.size(); ++metaObjectOffset)
            allowedRevisionCache[metaObjectOffset] = minorVersion;
    }
}
